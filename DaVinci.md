...
> 媒体优化：<br>
> 1. 播放->代理模式->更低分辨率<br>
> 2. 媒体优化功能：媒体池or时间线选中素材右键->生成优化媒体文件。点击以后达芬奇就会自动帮忙渲染，只不过通常你自己需要去修改一下渲染的编码和分辨率，这样文件就不会太大造成浪费。文件->项目设置->主设置->优化的媒体和渲染缓存。如果拍摄的是4k素材，建议分辨率设置为1/2或者1/4，编码可以选择DNxHR SQ如果是苹果电脑建议选择Prores 422LT，同时也需要注意缓存文件位置。<br>
> 预渲染：播放->渲染缓存->用户自定义或者智能。如果是用户自定义就需要回到时间线上选中卡顿的片段，然后右键选择你想要渲染的部分。<br>

# 智能媒体夹以及双时间线剪辑的逻辑
> 素材快速分类以及双时间线剪辑<br>
> 习惯：先专心剪辑Aroll，等到你仔细推敲你的内容本身敲定之后，再来加Broll。锁定Broll以后再来调色，加效果。这样第一可以避免重复劳动，第二可以防止过多的效果还有调色影响你时间线顺畅的回放。<br>
> 如何快速地挑出Aroll和Broll呢，使用DaVinci的智能媒体夹功能：在媒体池的智能媒体夹右键创建智能媒体夹，点击以后就会跳出来一个条件框（也就是自定义高级搜索），在这里可以筛选出同一天拍摄的素材、同一个设备拍摄的素材、或者是同一个场景拍摄的素材，只有两个素材之间有一个或多个相关的属性，就会把他们提取出来。<br>
